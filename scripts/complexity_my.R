...
#' @export
complexity_my <- function(...) {
  UseMethod("complexity_my")
}

#' @rdname complexity
#' @export
complexity_my.default <- function(x, y, groups="all", ...) {
...
#' @rdname complexity
#' @export
complexity_my.formula <- function(formula, data, groups="all", ...) {
  ...
  complexity_my.default(modFrame[, -1, drop=FALSE], modFrame[, 1, drop=FALSE],
                     groups, ...)
}

ls.complexity <- function() {
  c("overlapping_my", "neighborhood_my", "linearity.class_my")
}