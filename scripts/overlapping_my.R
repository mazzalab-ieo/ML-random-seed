...
#' @export
overlapping_my <- function(...) {
  UseMethod("overlapping_my")
}

#' @rdname overlapping
#' @export
overlapping_my.default <- function(x, y, measures="all", ...) {
...
#' @rdname overlapping
#' @export
overlapping_my.formula <- function(formula, data, measures="all", ...) {
  ...
  overlapping_my.default(modFrame[, -1, drop=FALSE], modFrame[, 1, drop=FALSE],
                      measures, ...)
}

ls.overlapping <- function() {
  c("F1","F1v","F2","F3","F4","F2_partial","F3_partial","F4_partial")
}
